...
export default function Historial() {
  ...
  // getAll() trae la lista con sin_detalle=true (más rápido para paginar),
  // así que pedido.items siempre llega vacío. Traer el detalle completo
  // bajo demanda con getById antes de abrir el modal o generar el ticket.
  async function obtenerDetalle(pedido) {
    setDetalleLoading(pedido.id_pedido)
    try {
      const res = await pedidosApi.getById(pedido.id_pedido)
      if (!res.ok) return pedido
      const data = Array.isArray(res.data) ? res.data[0] : res.data
      return data || pedido
    } catch {
      return pedido
    } finally {
      setDetalleLoading(id => (id === pedido.id_pedido ? null : id))
    }
  }

  async function verDetalle(pedido) {
    setDetalle(await obtenerDetalle(pedido))
  }

  async function descargarTicket(pedido) {
    generarTicketPDF(await obtenerDetalle(pedido))
  }
